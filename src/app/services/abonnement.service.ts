...
export class AbonnementService {
  ...
  /*renouvelerAbonnementParUser(idUser: number, duree: string, type: string, stripeToken: string) {
    ...
    getProfilIA(idUser: number): Observable<any> {
      return this.http.get<any>(`http://localhost:8081/Abonnement/analytics/predict-user-profil/${idUser}`);
    }
    predictRenewal(idUser: number): Observable<any> {
      return this.http.get<any>(`http://localhost:8081/Abonnement/analytics/predict-renewal/${idUser}`);
    }
