...
export class MesReservationsComponent implements OnInit {
  ...
  getReservationsByUserr(): void {
    this.http.get<Reservation[]>(
      `http://localhost:8080/PIdev/reservations/user/${this.userId}`,
      { headers: this.getHeaders() }
    )
  }
getReservationsByUser(userId: number) {
  this.http.get<any[]>(`http://localhost:8080/PIdev/reservations/user/${this.userId}`
  ).subscribe({
    next: (data) => {
  ...
  annulerReservation(reservationId: number): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http.delete(
          `http://localhost:8080/PIdev/reservations/annuler/${reservationId}`,
